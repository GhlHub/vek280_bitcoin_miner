...
module sha256_core_dsp_explicit #(
    parameter bit FOUR_PHASE = 1'b1
) (
    ...
    sha256_core_iterative #(
        .EXPLICIT_DSP_SCHEDULE(1'b1),
        .FOUR_PHASE(FOUR_PHASE)
    ) u_core (
        .clk_i(clk_i), .rst_ni(rst_ni), .start_i(start_i), .block_i(block_i), .h_i(h_i),
